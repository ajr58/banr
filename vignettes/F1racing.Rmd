...
vignette: >
  %\VignetteIndexEntry{Global Diet Carbon Footprint}
...
races <- read_csv("~/R/data303/banr/data-raw/races.csv")
status <- read_csv("~/R/data303/banr/data-raw/status.csv")
results <- read_csv("~/R/data303/banr/data-raw/results.csv", col_types = cols(number = col_character()))
```
...
## Revisions
```{r}
new_data <- joined_results %>%
  filter(status == "Mechanical" | status == "Accident" | status == "Engine" | status == "Brakes") %>%  # Filter by categories
  group_by(status, year)
```
...
plot_ly(new_data, 
               ...
               y = ~laps, 
               type = 'box') %>% 
  layout(title = "Spread of Failures over F1 Racing Laps",
         xaxis = list(title = "Race status"),
         yaxis = list (title = "Number of Laps"))
```
